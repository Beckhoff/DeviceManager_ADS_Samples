...
int main() {
	...
	std::string oldAmsNetId;;
	error = twincat->getAmsNetId(oldAmsNetId);
	handleError(error);

	const char* newAmsNetId = "5.80.201.233.1.1";
	error = twincat->setAmsNetId(newAmsNetId);
	handleError(error);

	std::cout << "Changed AmsNetId from " << oldAmsNetId << " to " << newAmsNetId << std::endl;
}